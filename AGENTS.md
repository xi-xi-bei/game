...
艾梅乙（艾滋病·梅毒·乙肝）育龄防护公益科普互动游戏。双视角（女性/男性）交互式文字选择类游戏，覆盖高危暴露、检测抉择、伴侣告知、婚检/孕前检/产检四大场景，三个结局等级。
...
│   │   ├── game-data-female.ts # 女性视角剧情数据
│   │   ├── game-data-male.ts   # 男性视角剧情数据
│   │   └── utils.ts        # 通用工具函数 (cn)
│   └── server.ts           # 自定义服务端入口
├── DESIGN.md               # 设计规范
...
- **剧情数据**：两个独立的 `PerspectiveData` 对象（`femaleData` / `maleData`），每个节点 `StoryNode` 含叙述、选项、科普弹窗
- **游戏流程**：开篇选择视角 → 节点导航（选项/自动跳转） → 科普弹窗 → 结局评级
